...
| **Distilled** — *Foundations of Scalable Systems* (Gorton, O'Reilly 2022) | Comprehensive textbook on scalable distributed systems — load balancing, caching, messaging, microservices resilience, sharding, eventual vs strong consistency, Kafka, Flink. Use the derived skills (`scalability-foundations`, `distributed-systems-essentials`, etc.) for implementation-depth questions. |
| **Distilled** — *Fundamentals of Software Architecture* (Richards/Ford, O'Reilly 2020) | Higher-level architectural style choice; complements Gorton (style) + Gorton (implementation). |
| **Distilled** — *Software Architecture: The Hard Parts* (Ford/Richards/Sadalage/Dehghani, O'Reilly 2021) | The trade-off analysis method; the 8 transactional saga taxonomy; data ownership / data access patterns; contract coupling vocabulary; architectural quanta. Eight derived skills (`distributed-workflows-and-sagas`, `trade-off-analysis-method`, `data-ownership-and-distributed-data`, `service-contracts-and-coupling`, `architectural-quanta-and-modularity`, `service-and-data-decomposition`, `code-reuse-in-distributed-systems`, `service-granularity-forces`) cover the book's content. **Load `trade-off-analysis-method` for any architecture decision someone will second-guess later.** |
