...
description: Quick-access management of Alex's content anti-patterns list — add, remove, append, or amend words, phrases, or entire pattern categories. Use when Alex says "add [word] to the off-limits list", "remove [word] — I actually use that", "add a new anti-pattern category for X", "update the anti-patterns", or "kill '[phrase]' across my content". Edits .claude/references/content-anti-patterns.md in place with minimal ceremony.
---
